...
def pull_or_clone(repo, directory='', branch='master', options='', run_as_user=None):
    cmd = 'git -C {dir} fetch origin && git -C {dir} checkout {branch} 2>/dev/null && git -C {dir} merge origin/{branch} --quiet 2>/dev/null || git clone {options} --quiet {repo} --branch {branch} {dir}'.format(repo=repo, branch=branch, dir=directory, options=options)
    sudo(cmd, user=run_as_user)
